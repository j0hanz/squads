...
def check_frontmatter(file_path, dir_name):
    ...
    if fm_end == -1:
        failures.append((file_path, "frontmatter not closed (no closing '---')"))
        return failures
    ...
    if not name_match:
        ...
        if name_value != dir_name:
            failures.append((file_path, f"name '{name_value}' does not match dir '{dir_name}'"))
    ...
    if not desc_match:
        ...
        if not desc_value.startswith('Use when'):
            failures.append((file_path, "description does not start with 'Use when'"))

        # Check 4: length <= 500
        if len(desc_value) > 500:
            failures.append((file_path, f"description is {len(desc_value)} chars (max 500)"))
...
def check_relative_links(file_path):
    ...
    for match in re.finditer(pattern, content):
        ...
        if not target_path.exists():
            failures.append((file_path, f"link to {target_file} (file does not exist)"))
            continue
        ...
        if anchor:
            ...
            if anchor not in headings:
                failures.append((file_path, f"link to {target_file}#{anchor} (anchor not found)"))
...
def main():
    ...
    if not skills_dir.exists():
        print(f"FAIL skills: directory not found")
        return 1
