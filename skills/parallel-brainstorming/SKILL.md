...
- **Scan:** Run `scan_context.py` with whichever Python interpreter is available — try `python3`, then `py`, then `python`: `<interp> ${CLAUDE_SKILL_DIR}/scripts/scan_context.py <noun1> <noun2> ... --cwd '<root>'` (fallback to `Grep` if it fails). Output is the compact Codebase Context Report JSON.
