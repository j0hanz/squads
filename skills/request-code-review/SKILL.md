...
1. Confirm the tests covering the diff pass — paste fresh test output. If any covering test fails, abort and report — never dispatch a review of a failing diff. If no tests cover the diff, say so explicitly and get user confirmation before proceeding; the dispatch summary must note the missing coverage.
2. Pick the review mode and resolve the diff:
   - **Committed** (target is a branch, commit, or path): resolve the default branch with `def=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null || echo origin/main)`; if `git rev-parse --verify "$def"` fails, try `origin/master`, then `origin/develop`; if none verify, abort and report "could not resolve default branch — pass an explicit base". Set `head` = the given branch or commit if one was passed (verify with `git rev-parse --verify`), else `HEAD`; set `base = git merge-base "$def" {{head}}` (or the given base). Run `git status --porcelain`; if dirty, abort and report. Capture `git diff {{base}}..{{head}}` (append `-- <path>` if a path was given).
