...
argument-hint: "[target: branch, commit, or path]"
---
...
1. Confirm the tests covering the diff pass — paste fresh test output.
2. Pick the review mode and resolve the diff:
   - **Committed** (target is a branch, commit, or path): resolve the default branch with `def=$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null || echo origin/main)`; if `git rev-parse --verify "$def"` fails, try `origin/master`, then `origin/develop`; if none verify, abort and report "could not resolve default branch — pass an explicit base". Set `base = git merge-base "$def" HEAD` (or the given base), `head = HEAD` (or the given commit). Run `git status --porcelain`; if dirty, abort and report. Capture `git diff {{base}}..{{head}}` (append `-- <path>` if a path was given).
...
1. Fill the dispatch prompt below, then dispatch one subagent with write/edit tools denied.
...
3. On **FAIL**: invoke `receive-code-review`. Do not fix findings directly.
